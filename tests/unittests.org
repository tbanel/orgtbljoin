...
(let ((org-calc-default-modes
  ...
  ;; Compute all keyboard macros
  (org-block-map
   (lambda ()
     (if (search-forward ":kbd-macro" (point-at-eol) t)
...
- answer
  : nut_with_header
  : type
which should transform exactly the "start" table into the "expected result" table.

#+begin_src elisp :results none :kbd-macro t
(execute-kbd-macro
 (kbd
  "C-c C-n C-c C-n <tab> C-k C-k C-c C-p <tab> C-k C-k C-y C-y <up> <tab> C-e M-<backspace> expected SPC result
 <up> <home> <tab> C-c C-n <down> <tab> <tab> M-x orgtbl-join <return> nut_with_h <tab> <return>
 <return> <return>"))
#+end_src
...
*** expected result
| quty | type     | units |  mul | Carb | Fiber | Sugar | Protein |
|------+----------+-------+------+------+-------+-------+---------|
|   70 | onion    |     5 |  350 |  9.0 |   1.3 |   4.4 |     1.3 |
|  120 | tomatoe  |     8 |  960 |  3.4 |   0.6 |   2.1 |     0.8 |
|  300 | eggplant |     2 |  600 |  8.6 |   2.5 |   3.2 |     0.8 |
|  300 | eggplant |     2 |  600 |  8.7 |   2.6 |   3.3 |     0.9 |
|  300 | eggplant |     2 |  600 |  8.5 |     ? |     ? |       ? |
|------+----------+-------+------+------+-------+-------+---------|
|  100 | tofu     |     1 |  100 |  1.4 |   0.7 |   0.5 |     6.6 |
|  250 | corn     |    15 | 3750 | 21.3 |   4.7 |   1.8 |     2.8 |
|   90 | tomatoe  |     5 |  450 |  3.4 |   0.6 |   2.1 |     0.8 |
|------+----------+-------+------+------+-------+-------+---------|
|   80 | amarante |     1 |   80 |
#+TBLFM: $4=$1*$3
...
(execute-kbd-macro
 (kbd
  "C-c C-n <tab> C-k C-k C-y C-y <up> <tab> C-e M-<backspace> expected SPC result C-c C-n <tab> C-k C-k C-c C-p <down> <tab> <tab>
 M-x orgtbl-join <return> nut_with_header <return>
        <return> type <return>
C-c C-p C-c C-p <tab>"))
#+end_src
